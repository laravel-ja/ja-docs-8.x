...
> 2ndパーティーパッケージとして取り入れられたCashier-mollieのドキュメントは、オリジナル英語並びに本翻訳リポジトリに含まれていません。このページのみ翻訳中です。そもそもCashierのドキュメント利用者は少ないため、mollieドキュメントはゆっくりと翻訳していきます。翻訳中は誤訳などのレポートはご遠慮ください。
